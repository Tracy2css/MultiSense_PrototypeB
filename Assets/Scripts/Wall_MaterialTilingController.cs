...
namespace VRUIP
{
    public class WallMaterialTilingController : MonoBehaviour
    {
        [Header("Target Object")]
        public WallInteractionController wallInteractionController; // Reference to WallInteractionController
        ...
        private void Start()
        {
            if (wallInteractionController == null)
            {
                DebugManager.Instance?.LogError("WallInteractionController reference is missing.");
                return;
            }

            // Subscribe to material change event
            wallInteractionController.OnMaterialChanged.AddListener(OnMaterialChanged);

            // Initialize UI with the current material
            InitializeUI(wallInteractionController.currentWallMaterial);
        }
        ...
        private void OnDestroy()
        {
            ...
            if (wallInteractionController != null)
            {
                wallInteractionController.OnMaterialChanged.RemoveListener(OnMaterialChanged);
            }
        }

        private void OnMaterialChanged(Material newMaterial)
        {
            currentWallMaterial = newMaterial;
            InitializeUI(currentWallMaterial);
        }
        ...
        private void InitializeUI(Material material)
        {
            if (material != null)
            {
                ...
                Vector2 initialTiling = currentWallMaterial.GetTextureScale("_BaseMap");
        ...
        private void SetTiling(float value)
        {
            ...
            {
                currentWallMaterial.SetTextureScale("_BaseMap", new Vector2(value, value));
                DebugManager.Instance?.Log($"Tiling updated: X={value}, Y={value}");

                // Update the tiling in WallInteractionController
                wallInteractionController.UpdateTiling(value);
            }
            else
            {
